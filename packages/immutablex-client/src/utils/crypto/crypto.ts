...
function deserializeSignature(sig: string, size = 64): SignatureOptions {
  const sigHexPrefixRemoved = encUtils.removeHexPrefix(sig);
  return {
    r: new BN(sigHexPrefixRemoved.substring(0, size), 'hex'),
    s: new BN(sigHexPrefixRemoved.substring(size, size * 2), 'hex'),
    recoveryParam: importRecoveryParam(sigHexPrefixRemoved.substring(size * 2, size * 2 + 2)),
  };
}
