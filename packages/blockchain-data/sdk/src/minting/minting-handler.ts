...
import { MintRequest, MintingPersistence } from './persistence/type';
...
export const mint = async <DBClient>(
  mintingPersistence: MintingPersistence<DBClient>,
  blockchainDataSDKClient: BlockchainData,
  { defaultBatchSize = 100, chainName = 'imtbl-zkevm-testnet', maxNumberOfTries = 3 },
  ...
  while (true) {
    ...
    let batchSize = Math.min(
      mintingResponse ? parseInt(mintingResponse.imx_remaining_mint_requests, 10) : defaultBatchSize,
      defaultBatchSize,
      100
    );
    ...
    const preContractAddressAssets = pendingMints.reduce((acc, row) => {
      ...
      Object.entries(preContractAddressAssets).map(
        async ([contractAddress, assets]) => {
          ...
          try {
            ...
            logger.info(`mintingResponse: ${JSON.stringify(response, null, 2)}`);
            ...
            if (e.code === 'CONFLICT_ERROR' && e.details?.id === 'reference_id') {
              ...
              const { assetsToRetry, assetsExceededMaxNumberOfTries } = assets.reduce((acc, { tried_count = 0, id }) => {
                ...
                return acc;
              }, { assetsToRetry: [], assetsExceededMaxNumberOfTries: [] } as {
    ...
    mintingResponse = mintingResults.reverse().find((r): r is PromiseFulfilledResult<Types.CreateMintRequestResult> => r.status === 'fulfilled')?.value;
  }
};
...
export const processMint = async <DBClient>(
  ...
  const mintReq = await mintingPersistence.getMintingRequest(contractAddress, referenceId);
  if (!mintReq) {
    logger.info(`minting request not found in the database, ${JSON.stringify(event.data)}`);
  }
