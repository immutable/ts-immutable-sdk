...
export interface SequenceTransactionRequest {
  to?: string;
  data?: Hex.Hex | string | null;
  value?: bigint;
  nonce?: bigint;
  chainId?: bigint | number;
}

export type SequenceTransactionParams = {
...
const buildMetaTransaction = async (
  transactionRequest: SequenceTransactionRequest,
...
export const prepareAndSignTransaction = async ({
...
}: SequenceTransactionParams & { transactionRequest: SequenceTransactionRequest }): Promise<{ to: Address.Address; data: Hex.Hex; }> => {
  const user = await auth.getUser();
  if (!user?.accessToken) {
    throw new JsonRpcError(
      ProviderErrorCode.UNAUTHORIZED,
  ...
  const response = await fetch('https://api.dev.immutable.com/relayer-mr/v1/build-guest-module-calldata', {
    method: 'POST',
    headers: { 
...
const signMetaTransaction = async (
  ...
  const wallet = await createWallet(auth, Address.from(walletAddress), sequenceSigner);
...
async function fetchDeploymentSalt(
  ...
  const apiUrl = 'https://api.dev.immutable.com/passport-mr/v1/counterfactual-salt';
