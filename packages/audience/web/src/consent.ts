import type { ConsentLevel, ConsentStatus, Environment } from './types';
import {
  getConsentCookie,
  setConsentCookie,
  deleteCookie,
...
export class ConsentManager {
  ...
  async fetchServerConsent(anonymousId: string): Promise<ConsentStatus | undefined> {
    try {
      const url = `${this.baseUrl}${CONSENT_ENDPOINT}?anonymousId=${encodeURIComponent(anonymousId)}`;
  ...
  private async syncToServer(anonymousId: string, status: ConsentLevel): Promise<void> {
    try {
      await fetch(`${this.baseUrl}${CONSENT_ENDPOINT}`, {
        method: 'PUT',
        headers: {
