...
import { ConsentManager, DebugLogger } from '@imtbl/audience-sdk';
...
import { webConsentTransport } from './consent-transport';
 ...
 *
 * Create via `ImmutableWebSDK.init()`. Call `shutdown()` when done.
 */
export class ImmutableWebSDK {
  ...
  static init(config: WebSDKConfig): ImmutableWebSDK {
    ...
    if (ImmutableWebSDK.liveInstances > 0) {
    ...
    ImmutableWebSDK.liveInstances += 1;
  ...
  private isTrackingDisabled(): boolean {
    return this.consent.getLevel() === 'none';
  }
  ...
  page(properties?: Record<string, unknown>): void {
    ...
    renewSession(this.cookieDomain);
  ...
  setConsent(level: ConsentLevel): void {
    ...
    this.consent.setLevel(level, this.anonymousId, {
      // Note: session_end is intentionally not emitted on consent revocation.
      // The queue is purged immediately — no events should be sent after opt-out.
      onPurgeQueue: () => {
        this.queue.stop();
        this.queue.clear();
      },
  ...
  shutdown(): void {
    ...
    this.queue.destroy();
    ImmutableWebSDK.liveInstances = Math.max(0, ImmutableWebSDK.liveInstances - 1);
  }
}
