...
- `@imtbl/audience-core` is **bundled inline** via the `tsdown.config.ts` alias, not externalised. Changes to `core` count toward this budget — that's why the workflow triggers on `core/**` paths too.
