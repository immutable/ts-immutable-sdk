...
describe('provider validation', () => {
  const mockRequestFunc = (request: { method: string, params?: any[] | undefined }):
  ...
  describe('isWeb3Provider', () => {
    ...
    it('should return false when it is not a Web3Provider', () => {
      ...
      const web3Provider = { request: mockRequestFunc } as unknown as Web3Provider;
      const result = isWeb3Provider(web3Provider);
  ...
  describe('validateProvider', () => {
    ...
    it('should throw an error if the underlying provider is on an unsupported network', async () => {
      ...
      const testWeb3Provider = new Web3Provider(underlyingProviderMock, ChainId.ETHEREUM);
      const validationOverrides = { allowMistmatchedChainId: true, allowUnsupportedProvider: false };
      await expect(validateProvider(testCheckoutConfig, testWeb3Provider, validationOverrides))
        .rejects.toThrowError(
    ...
    it('should not throw an error if allowUnsupportedProvider true and underlying network is unsupported', async () => {
      ...
      const validationOverrides = { allowMistmatchedChainId: false, allowUnsupportedProvider: true };
      expect(await validateProvider(testCheckoutConfig, testWeb3Provider, validationOverrides)).toBe(testWeb3Provider);
    });
  });
});
