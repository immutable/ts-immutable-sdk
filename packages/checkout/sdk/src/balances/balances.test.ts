    ...
    it('should throw an error if the contract address is invalid', async () => {
      ...
      await expect(
        ...
        new CheckoutError(
          ...
          '[GET_ERC20_BALANCE_ERROR] Cause:provider is required to use ENS name as contract address (operation="new Contract", code=UNSUPPORTED_OPERATION, version=contracts/5.7.0)',
          CheckoutErrorType.GET_ERC20_BALANCE_ERROR,
        ),
      );
    });
