...
import {
  ALCHEMY_PATH, CHECKOUT_API_BASE_URL, ChainId, ENVIRONMENT_L1_CHAIN_MAP,
} from '../types';
...
export class CheckoutApiServiceL1RpcNode {
  ...
  async getTokenBalances({ walletAddress }: GetTokenBalances): Promise<GetTokenBalancesResponse> {
    let response;
    try {
      ...
      throw new Error(`Error fetching getTokenBalances for ${walletAddress}: ${error.message}`);
    }
  ...
  async getTokenMetadata({ token }: GetTokenMetadataParams): Promise<GetTokenMetadataResponse> {
    let response;
    try {
      response = await axios.post(
        // @ts-ignore -- this is needed because we do not have a way to get L1 only chains
      ...
      throw new Error(`Error fetching getTokenMetadata for ${token}: ${error.message}`);
    }
