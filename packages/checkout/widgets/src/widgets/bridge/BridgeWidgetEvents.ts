import { BridgeEvent, BridgeEventType, BridgeSuccess, IMTBLWidgetEvents, BridgeFailed } from "@imtbl/checkout-ui-types";

export const sendBridgeSuccessEvent = () => {
  const successEvent = new CustomEvent<BridgeEvent<BridgeSuccess>>(IMTBLWidgetEvents.IMTBL_BRIDGE_WIDGET_EVENT , {
    detail: {
...
export const sendBridgeFailedEvent = (reason: string) => {
  const failedEvent = new CustomEvent<BridgeEvent<BridgeFailed>>(IMTBLWidgetEvents.IMTBL_BRIDGE_WIDGET_EVENT, {
    detail: {
...
export function sendBridgeWidgetCloseEvent(){
  ...
  const closeWidgetEvent = new CustomEvent<BridgeEvent<any>>(IMTBLWidgetEvents.IMTBL_BRIDGE_WIDGET_EVENT, {
    detail: {
