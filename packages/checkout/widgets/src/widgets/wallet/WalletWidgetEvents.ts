...
import {IMTBLWidgetEvents, WalletEvent, WalletEventType, WalletNetworkSwitchEvent} from "@imtbl/checkout-ui-types";

export function sendWalletWidgetCloseEvent(){
  console.log(WalletEventType.CLOSE_WIDGET)
  const closeWidgetEvent = new CustomEvent<WalletEvent<any>>(IMTBLWidgetEvents.IMTBL_WALLET_WIDGET_EVENT , {
    detail: {
...
export function sendNetworkSwitchEvent(network: NetworkInfo){
  const successWalletWidgetEvent = new CustomEvent<WalletEvent<WalletNetworkSwitchEvent>>(IMTBLWidgetEvents.IMTBL_WALLET_WIDGET_EVENT , {
    detail: {
