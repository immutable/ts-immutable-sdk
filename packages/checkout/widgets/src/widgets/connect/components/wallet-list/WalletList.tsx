...
export const WalletList = () => {
  ...
  return(
    <Box testId="wallet-list" sx={{width: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-start'}}>
      <MenuItem testId={`wallet-list-${ConnectionProviders.METAMASK}`} elevated size="medium" onClick={() => onWalletClick(ConnectionProviders.METAMASK)}>
        ...
        <MenuItem.Label size="medium">{wallets[ConnectionProviders.METAMASK].heading}</MenuItem.Label>
        <MenuItem.IntentIcon></MenuItem.IntentIcon>
        <MenuItem.Caption>{wallets[ConnectionProviders.METAMASK].description}</MenuItem.Caption>
      </MenuItem>
    </Box>
  )
}