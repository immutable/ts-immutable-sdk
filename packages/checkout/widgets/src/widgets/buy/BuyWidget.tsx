...
import {ChainId, Checkout, ConnectionProviders, Transaction} from "@imtbl/checkout-sdk-web";
import { Web3Provider } from '@ethersproject/providers'
import {BigNumber} from 'ethers';
import AssetDetails from './AssetDetails';
import BuyButton from './BuyButton';
import { sendBuyWidgetCloseEvent, sendBuyWidgetNotConnectedEvent } from './BuyWidgetEvents';
...
class Asset {
  public static async getAsset(tokenAddress: string, tokenId: string): Promise<GetAssetResponse> {
    return {
...
export class Orderbook {
  ...
  public async createOrder(chainId: ChainId, orderId: string): Promise<Transaction> {
  ...
  public async getOrder(chainId: number, orderId: string): Promise<GetOrderResponse> {
    const imxToken = {
      address: "",
      name: "Immutable X",
      symbol: "IMX",
...
export function BuyWidget({params: { providerPreference, orderId }, theme}:BuyWidgetProps) {
  ...
  useEffect(() => {
    const buyWidgetSetup = async () => {
      if ((await checkout.checkIsWalletConnected({ providerPreference })).isConnected){
        let connectResult = await checkout.connect({providerPreference});
        const chainId = connectResult.network.chainId;
        ...
        const asset = await Asset.getAsset(order.sell.token, order.sell.tokenIdentifier)
      ...
      <Box sx={BuyWidgetStyle}>
        <Box sx={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
          ...
          <Button size={'small'} sx={{alignSelf:'flex-end'}}
              testId='close-button'
              onClick={sendBuyWidgetCloseEvent}>x</Button>
          </Box>
