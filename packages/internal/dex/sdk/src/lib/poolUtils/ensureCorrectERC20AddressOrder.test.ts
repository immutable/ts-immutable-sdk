import { WIMX_TEST_TOKEN, WETH_TEST_TOKEN } from 'test/utils';
...
describe('ensureCorrectERC20AddressOrder', () => {
  describe('given a pair that is not ordered', () => {
    it('orders the pair correctly', () => {
      const unorderedERC20Pair: ERC20Pair = [WIMX_TEST_TOKEN, WETH_TEST_TOKEN];
      ...
      expect(orderedERC20Pair[1].address).toEqual(WIMX_TEST_TOKEN.address);
    });
  });
});
