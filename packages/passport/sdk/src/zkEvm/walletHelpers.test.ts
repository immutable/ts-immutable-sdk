...
describe('walletHelpers', () => {
  ...
  describe('signMetaTransactions', () => {
    ...
    it('should correctly generate the signature for a given transaction', async () => {
      ...
      mockMagicTeeAdapter.personalSign.mockResolvedValue('0x904a25850e09260d88f3fc46fab4901e7c979fc583fe9d30a12c51ba5636355a1351b8ce823f765568d8b88cddd9c8ede9f1cc17dfd7ca953e05ecbbbdf8f51e');
      ...
      const signature = await signMetaTransactions(
        ...
        BigInt(chainId),
        walletAddress,
        mockMagicTeeAdapter as unknown as MagicTeeAdapter,
      );

      expect(signature).toBe('');
    });
  });
  ...
  describe('signAndPackTypedData', () => {
    ...
    it('should correctly generate the signature for a given typed data payload', async () => {
      ...
      mockMagicTeeAdapter.personalSign.mockResolvedValue('0xaec95114a3b8cf3c9693177a2abd8321cf775366a6c6aadf5953e082680fd90c6cb44972a1635b5e9f7f02490a47425be37a1965a6cbaaaa64404cb2cf3880f71c');
    ...
    describe('when the EOA address is smaller than the Immutable signer address', () => {
      it('should include the EOA signature in the combined signature first', async () => {
        ...
        const mockSignature = '0xaec95114a3b8cf3c9693177a2abd8321cf775366a6c6aadf5953e082680fd90c6cb44972a1635b5e9f7f02490a47425be37a1965a6cbaaaa64404cb2cf3880f71c';
        mockMagicTeeAdapter.personalSign.mockResolvedValue(mockSignature);

        const result = await signAndPackTypedData(
          typedDataPayload,
          relayerSignature,
          lowAddressSigner.address,
          BigInt(chainId),
          walletAddress,
          mockMagicTeeAdapter as unknown as MagicTeeAdapter,
        );

        const eoaSignatureWithoutPrefix = mockSignature.slice(2); // Remove leading `0x`
