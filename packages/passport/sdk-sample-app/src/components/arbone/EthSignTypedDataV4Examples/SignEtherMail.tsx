...
import { getEtherMailTypedPayload } from '../../zkevm/EthSignTypedDataV4Examples/etherMailTypedPayload';
...
function SignEtherMail({ disabled, handleExampleSubmitted }: RequestExampleProps) {
  ...
  useEffect(() => {
    const populateParams = async () => {
      if (arbOneProvider) {
        const chainIdHex = await arbOneProvider.request({ method: 'eth_chainId' });
  ...
  }, [address, arbOneProvider]);

  useEffect(() => {
    const getAddress = async () => {
      if (arbOneProvider) {
        const [walletAddress] = await arbOneProvider.request({
