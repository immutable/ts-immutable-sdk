      ...
      try {
        ...
        if (response.data.result.length > 0) {
          if (response.data.result[0].status === "failed") {
            ...
            const updates = await prisma.imAssets.updateMany({
              where: { assetId: uuid },
              data: { mintingStatus: null, assetId: newUUID },
            });

            if (updates.count > 0) {
              mintByMintingAPI(serverConfig[environment].collectionAddress, mint.ownerAddress, newUUID, serverConfig[environment].metadata);
            }
          }
        } else {
