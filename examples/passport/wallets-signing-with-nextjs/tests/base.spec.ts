...
test.describe("home page", () => {
  test("has title, heading and link", async ({ page }) => {
    await expect(page).toHaveTitle("Passport Wallets Signing");

    await expect(page.getByRole("heading", { name: "Passport Wallets Signing Examples" })).toBeVisible();
...
test.describe("sign message with EIP-712", () => {
  test("has heading, login button and initial account status set correctly", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Passport Wallets - Sign EIP-712 message" })).toBeVisible();
...
test.describe("sign message with ERC-191", () => {
  test("has heading, login button and initial account status set correctly", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Passport Wallets - Sign ERC-191 message" })).toBeVisible();
