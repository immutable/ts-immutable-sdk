...
import {signAndSubmitApproval, signListing, createListing} from "@/app/utils/listing";
import {
    Box,
    Select,
    TextInput,
...
export default function ConnectWithEtherJS() {
    ...
    const [sellItemContractAddress, setSellItemContractAddressState] = useState<string>('');

    // setup the sell item token ID state
    const [sellItemTokenID, setSellItemTokenIDState] = useState<string>('');

    // setup the sell item quantity state
    const [sellItemQty, setSellItemQtyState] = useState<string>('');
    ...
    const [showBuyItemContractAddress, setShowBuyItemContractAddressState] = useState<boolean>(false);

    // setup the buy item contract address state
    const [buyItemContractAddress, setBuyItemContractAddressState] = useState<string>('');
    ...
    const handleSellItemTokenQtyChange = (event: any) => {
        setSellItemQtyState(event.target.value);
    };

    const handleBuyItemTypeChange = (val: any) => {
        setBuyItemTypeState(val);
        val === 'ERC20' ? setShowBuyItemContractAddressState(true): setShowBuyItemContractAddressState(false);
    };
    ...
    const createER1155Listing = async () => {
        ...
        try {
            ...
            // create the listing
            const listingID = await createListing(orderbookSDK, preparedListing, orderSignature);
    ...
    return (
        <Box>
            <Box sx={{marginBottom: 'base.spacing.x5'}}>
                <Heading size="medium" sx={{marginBottom: 'base.spacing.x5'}}>Passport</Heading>
                <Grid>
            ...
            <Box>
                <Heading size="medium" sx={{marginBottom: 'base.spacing.x5'}}>Create ERC1155 listing</Heading>
                {successMessage && <Box sx={{color:'green', fontSize:'15', marginBottom: 'base.spacing.x5'}}>{successMessage}</Box>}
                {listingError && <Box sx={{color:'red', marginBottom: 'base.spacing.x5'}}>{listingError}</Box>}
                ...
                <FormControl sx={{marginBottom: 'base.spacing.x5'}}>
                    ...
                    <TextInput onChange={handleSellItemTokenQtyChange} />
                </FormControl>
                <FormControl sx={{marginBottom: 'base.spacing.x5'}}>
