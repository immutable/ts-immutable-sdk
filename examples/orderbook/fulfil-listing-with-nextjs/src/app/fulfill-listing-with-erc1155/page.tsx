...
export default function FulfillERC1155WithPassport() {
  ...
  useMemo(async () => {
    ...
    const filtered = listings.filter(
        listing => listing.accountAddress !== accountsState[0] && listing.sell[0].type === "ERC1155"
    );
  ...
  const executeTrade = async (listingID: string, rowIndex: number) => {
    const units = unitsToFill.rowIndex === rowIndex ? unitsToFill.units : undefined;
  ...
  const fulfillERC1155Listing = async (listingID: string, unitsToFill? : string) => {
      const { actions } = await orderbookSDK.fulfillOrder(
          listingID,
          accountsState[0],
          [{
            amount: '1000000',  // Insert taker ecosystem/marketplace fee here
            recipientAddress: '0xdbc7a16ee0837d994e5edd9b3c56c1a9f9d1c933', // Replace address with your own marketplace address
          }],
          unitsToFill
      );
          ...
          <Table.Body>
            {
                listings.map((listing: any, index: number) => {
                    ...
                    <Table.Row key={index}>
                        ...
                        <Table.Cell>
                          <FormControl sx={{ marginBottom: "base.spacing.x5" }}>
                            <TextInput onChange={(event: any) => handleUnitsToFillChange(index, event.target.value)}/>
                          </FormControl>
                        </Table.Cell>
                        <Table.Cell>
                          <Button
