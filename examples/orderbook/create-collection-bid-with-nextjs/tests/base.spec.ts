...
test.describe("create collection bid with ERC721", () => {
  test("loads creation screen", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Create.*Collection.*Bid.*ERC721/i })).toBeVisible();
  });
});
...
test.describe("create collection bid with ERC1155", () => {
  test("loads creation screen", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Create.*Collection.*Bid.*ERC1155/i })).toBeVisible();
  });
});
