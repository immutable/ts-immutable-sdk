...
test.describe("home page", () => {
  test("has title, heading and fulfillment links", async ({ page }) => {
    await expect(page).toHaveTitle("Orderbook SDK - Fulfill listing with NextJS");
    await expect(page.getByRole("heading", { name: "Orderbook - Fulfill listing" })).toBeVisible();
    await expect(page.getByTestId("fulfill-listing-with-erc721")).toBeVisible();
    await expect(page.getByTestId("fulfill-listing-with-erc1155")).toBeVisible();
  });
});
