...
export async function waitForCollectionBidToBeOfStatus(
  ...
  await retry(async () => {
    const { result } = await orderBookSdk.getCollectionBid(collectionBidId);
    ...
    if (!equalStatus || !equalFillStatus) {
      throw new Error(
        `Collection bid ${collectionBidId} is of status [${JSON.stringify({
